...
public class WatchApiController {
    ...
    @ApiResponse(responseCode = "200", description = "스마트워치 목록 조회 성공")
    @GetMapping
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "스마트워치 삭제 성공"),
            @ApiResponse(responseCode = "404", description = "해당 스마트워치 없음")
    })
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "스마트워치 조회 성공"),
            @ApiResponse(responseCode = "404", description = "해당 스마트워치 없음")
    })
    @GetMapping("/{uuid}")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "스마트워치 등록 성공"),
            @ApiResponse(responseCode = "409", description = "중복된 UUID")
    })
    @PostMapping
    ...
    @Operation(summary = "웹에서 스마트워치 정보 수정", description = "웹을 통해 특정 스마트워치 정보를 수정합니다.")
    ...
    @Operation(summary = "웹에서 스마트워치 목록 조회", description = "웹을 통해 등록된 모든 스마트워치 목록을 조회합니다.")
    @GetMapping("/web")
    ...
    @Operation(summary = "웹에서 스마트워치 조회 (ID)", description = "스마트워치 ID를 이용하여 특정 스마트워치를 조회합니다.")
    @GetMapping("/web/getWatch/{watchId}")
    public ResponseEntity<SuccessResponse<?>> findWatchByIdForWeb(
            @Parameter(description = "조회할 스마트워치 ID", example = "1") @PathVariable Long watchId) {
        final WatchResponseForWebDto responseDto = watchService.findWatchByIdForWeb(watchId);
        return SuccessResponse.ok(responseDto);
    }

    @Operation(summary = "웹에서 스마트워치 조회 (UUID)", description = "스마트워치 UUID를 이용하여 특정 스마트워치를 조회합니다.")
    @GetMapping("/web/{uuid}")
    ...
    @Operation(summary = "접촉 금지 대상 스마트워치 목록 조회", description = "접촉 금지 대상 스마트워치 목록을 조회합니다.")
    ...
    @Operation(summary = "접촉 금지 및 금지된 위치 조회", description = "특정 스마트워치의 접촉 금지 대상 및 금지된 위치 정보를 조회합니다.")
    @GetMapping("/web/getNoContactAndProhibitedIdWithName/{watchId}")
    public ResponseEntity<SuccessResponse<?>> getNoContactAndProhibitedIdWithName(
            @Parameter(description = "조회할 스마트워치 ID", example = "1") @PathVariable Long watchId) {
        NoContactAndProhibitedIdWithNameDto responseDto = watchService.getNoContactAndProhibitedIdWithName(watchId);
        return SuccessResponse.ok(responseDto);
    }

    @Operation(summary = "스마트워치 정보 이관", description = "스마트워치 정보를 다른 스마트워치로 이관합니다.")
